...
colors = [
    'White', 'Black', 'Cream', 'Blue', 'Navy', 'Brown', 'Grey',
    'Olive Green', 'Maroon', 'Sand Beige', 'Charcoal'
]

if len(sorted_boxes) == 11:
    for i, b in enumerate(sorted_boxes):
        x, y, w, h = b
        cropped = img[y:y+h, x:x+w]
        color_name = colors[i].replace(' ', '-')
        out_name = f"cp1-{color_name}_v6.jpg"
        cv2.imwrite(os.path.join(out_dir, out_name), cropped, [cv2.IMWRITE_JPEG_QUALITY, 95])
        print(f"Saved {out_name}")
else:
    print(f"Warning: Found {len(sorted_boxes)} regions, expected 11.")
